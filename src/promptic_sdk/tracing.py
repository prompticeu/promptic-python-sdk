...
_promptic_tracing_configured = False


def is_tracing_configured() -> bool:
    """Return whether this SDK installed the active Promptic trace exporter."""
    return _promptic_tracing_configured
...
def init(
    *,
    ...
    """
    global _configured_api_key, _configured_endpoint, _promptic_tracing_configured
    ...
    _promptic_tracing_configured = True
    _configure_artifacts_once(api_key=api_key, endpoint=endpoint)
...
def _langsmith_tracing_context(
    ...
    ``promptic.ai_component`` / ``promptic.dataset.id`` attributes needed to
    link traces to AI components.
    """
...
def ai_component(
    ...
    *,
    ...
    langsmith_ctx = _langsmith_tracing_context(name, effective_dataset_id)
