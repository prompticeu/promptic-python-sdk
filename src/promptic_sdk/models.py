...
class _AvailableModelRequired(TypedDict):
    """Fields present on every available-model record."""

    id: str
    name: str
    provider: str
    group: Literal["openai", "platform", "google", "custom", "openrouter"]
    judgeEligible: bool


class AvailableModel(_AvailableModelRequired, total=False):
    """Model available to the authenticated AI Application."""

    region: Literal["eu", "us", "global"]


class AvailableModelList(TypedDict):
    """Effective model catalog for the authenticated AI Application."""
