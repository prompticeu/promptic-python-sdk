...
submissions from infrastructure you control. They use your normal Promptic
login or API key by default:

```python
from promptic_sdk import AgentGymClient, AgentGymCaseResult

def candidate(case):
    return AgentGymCaseResult.structured(my_agent(case.input, case.files))
...
Run `promptic login` once, or configure `PROMPTIC_API_KEY`. The high-level
`submit()` method creates a revision-bound session, materializes all cases,
invokes the callback, uploads returned artifacts, links traces, finalizes the
predictions, and waits for scoring.

The same flow is available from the CLI:

```bash
promptic gym run BENCHMARK_ID my_agent:run \
  --name my-agent \
  --version 1.0.0
```
...
run it in isolation, collect its predictions and files, and let a trusted
runner use the lower-level session API to upload and finalize them.
...
See [`examples/agent_gym_simple.py`](examples/agent_gym_simple.py) for the
recommended API. See
[`examples/agent_gym_external_submission.py`](examples/agent_gym_external_submission.py)
for the lower-level protocol including explicit trace handling.
